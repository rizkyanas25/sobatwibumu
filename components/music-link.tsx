...
const palettes = {
  "apple-music": {
    wrapper: "bg-[#FA243C] text-white shadow-[0_18px_50px_rgba(250,36,60,0.24)]",
    button: "bg-black/12 text-white",
    label: "Apple Music",
  },
  spotify: {
    wrapper: "bg-[#1DB954] text-[#03150A] shadow-[0_18px_50px_rgba(29,185,84,0.24)]",
  ...
  return (
    ...
    >
      ...
      <div className="relative flex min-w-0 items-center gap-3">
        <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl ${palette.button}`}>
          <Icon className="h-5 w-5" />
        </div>
        <div className="min-w-0">
          <p className="text-[11px] font-medium uppercase tracking-[0.28em] opacity-65">{palette.label}</p>
          <p className="truncate text-base font-semibold sm:text-lg">{`Dengerin di ${palette.label}`}</p>
        </div>
      </div>

      <div className={`relative flex h-10 w-10 shrink-0 items-center justify-center rounded-full ${palette.button}`}>
